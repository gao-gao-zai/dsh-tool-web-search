...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    'settings.plugin.item': { kind: 'keyed'; scope: 'root'; keyProps: Record<string, object> }
  }
}

interface WebSearchSettingsScope {
  getSnapshot(): { status: string; value: Record<string, any> | undefined; writable: boolean }
...
const styles = {
  ...
  input: { minHeight: 34, boxSizing: 'border-box', padding: '6px 8px', border: '1px solid var(--dsw-alias-border-l2)', borderRadius: 4, background: 'var(--dsw-alias-fill-control)', color: 'var(--dsw-alias-label-primary)' },
  actions: { display: 'flex', justifyContent: 'flex-end', gap: 8 },
  button: { minHeight: 34, padding: '6px 12px', border: '1px solid var(--dsw-alias-border-l2)', borderRadius: 4, background: 'var(--dsw-alias-fill-control)', color: 'var(--dsw-alias-label-primary)', cursor: 'pointer' },
  primary: { background: 'var(--dsw-alias-state-business-primary)', color: 'white', borderColor: 'var(--dsw-alias-state-business-primary)' },
  error: { color: 'var(--dsw-alias-state-danger)', fontSize: 13 },
}

function mergeConfig(value: Record<string, any> | undefined): Record<string, any> {
  return {
    ...DEFAULTS,
    ...(value || {}),
    bing: { ...DEFAULTS.bing, ...(value?.bing || {}) },
    searxng: { ...DEFAULTS.searxng, ...(value?.searxng || {}) },
  }
}
...
function InputField(props: any) {
  ...
  return React.createElement('label', { style: styles.field },
    ...
    React.createElement('input', {
      ...
      max,
      step,
      onChange: (event: React.ChangeEvent<HTMLInputElement>) => onChange(type === 'number' ? Number(event.target.value) : event.target.value),
    }),
...
function CheckField(props: any) {
  ...
  return React.createElement('label', { style: { ...styles.field, display: 'flex', gridTemplateColumns: 'auto 1fr', alignItems: 'center', gap: 8 } },
    React.createElement('input', { type: 'checkbox', checked: Boolean(checked), onChange: (event: React.ChangeEvent<HTMLInputElement>) => onChange(event.target.checked) }),
    React.createElement('span', { style: styles.label }, label),
  )
}

function WebSearchSettingsCard(props: { scope: WebSearchSettingsScope }) {
  const { scope } = props
  ...
  const update = (path: string, value: any) => {
    setDraft((current: Record<string, any>) => {
